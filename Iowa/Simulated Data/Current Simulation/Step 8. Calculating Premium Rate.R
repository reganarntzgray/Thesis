...
Yield <- read.table("Yield_current_sim_Iowa")
...
write.table(SD_yields,"SD_yields_cur_Iowa")
...
yield <- yield_data[yield_data$year %in% current,3]
...
Premiums_Iowa_current <- data.frame(county,premium_dyn,premium_stat,ratio)

write.table(Premiums_Iowa_current,"Premiums_Iowa_current")

install.packages("stargazer")
library(stargazer)

stargazer(Premiums_Iowa_current, nobs=F,summary.logical = F, flip=TRUE)
