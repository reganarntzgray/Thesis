...
ontario <- readRDS("Data needed to complete empirical work/Daily weather data/Ontario/weather_raw_daily.rds")
df <- ontario[order(ontario$county),]
df <- df[order(df$year),]

df <- df[df$days %in% c(1:59,61:366),]
...
#calculating gdd according to the agronomic method
...
daily_weather_ontario <- data.frame(year,days,county,min,max,pcp,vpd,gdd_ag,gdd,hdd,cdd,fdd,CHU)

saveRDS(daily_weather_ontario, file = "Data needed to complete empirical work/Daily weather data/Ontario/daily_weather_ontario")
