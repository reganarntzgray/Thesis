...
weather2 <- readRDS("C:/Users/Regan/Google Drive/Thesis Presentations and Proposal/Thesis/Data needed to complete empirical work/Daily weather data/Ontario/daily_weather_Ontario")


plant_silk <- read.table("future_sim_planting_silking")

county <- unique(weather2$county)
county <- rep(county,each=365)
county <- rep(county,times=100)

weather[,2]<-county

counties <- unique(plant_silk$county)
...
# year is 2090 - this is 141
time <- rep(141,N*T)

#trend is after 1999 - which is 49, so it is 91
trend <- rep(91,N*T)

  
#gdd, hdd, accum pcp and vpd for each month#

Data_d_iii_ont_sim_future <- data.frame(year,county,time,trend,gdd_season,hdd_season,mean_vpd_may,mean_vpd_june,mean_vpd_july,mean_vpd_aug,mean_pcp_may,mean_pcp_june,mean_pcp_july,mean_pcp_aug)
saveRDS(Data_d_iii_ont_sim_future,"Data_d_iii_ont_sim_future")
