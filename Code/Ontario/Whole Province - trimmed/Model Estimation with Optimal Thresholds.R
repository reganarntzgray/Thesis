...
W <- read.delim("Code/Ontario/W_ONT_trim_norm.txt",header=F)
W <- as.matrix(W)
...
X11 <- as.matrix(cbind(df[,4:ncol(df)],Dummy[,1:18]))
...
stargazer(M11,M12,M13,M14,type="html",title = "Ontario - Corn Producers: Regression results with optimal threshold levels",se=list(robust_M1,robust_M2,robust_M3,robust_M4), covariate.labels = c("T","Trend","GDD","HDD","VPD$_{May}$","VPD$_{June}$","VPD$_{July}$","VPD$_{August}$","PCP$_{May}$","PCP$_{June}$","PCP$_{July}$","PCP$_{August}$",c(1:18),"PCP$_{{july}_{sp_{low}}}$","PCP$_{{july}_{sp_{high}}}$","PCP$_{{july}_{sp}}$"),align=TRUE,out = "Result tables/Ontario_trim_threshold_results.html")
