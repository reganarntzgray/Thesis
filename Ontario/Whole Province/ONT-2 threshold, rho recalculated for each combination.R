...
{print(j)
  ...
  bh <- seq(max((Q3-ah[j]),((Q3-ah[j])/T)),min((max-ah[j]),((max-ah[j])/T)),.3)
 
  for(i in 1:length(al))
  {print(i)
    
    bl <- seq(max((min-al[i]),((min-al[i])/T)),min((Q3-al[i]),((Q3-al[i])/T)),.3)
...
al <- seq((a_l-5),(a_l+5),3)
ah <- seq((a_h-5),(a_h+5),3)
...
#> which(SSEmat == min(SSEmat), arr.ind = TRUE)
#row col
#[1,]  15   3
...
#> x
#[1]  12.000000 137.250000   1.228125  -0.487500
#> SSE(x)
#[1] 120619
